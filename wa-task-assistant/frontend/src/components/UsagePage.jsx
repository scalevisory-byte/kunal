...
const rupees = (usd, rate) => `₹${(usd * rate).toFixed(usd * rate >= 10 ? 0 : 2)}`;
    ...
    <div className="usage">
      <section className="kpis">
        {[
          ...
          <div key={c.key} className="kpi static">
            ...
            <span className="kpi-num">{money(c.usd)}</span>
            <span className="kpi-note">≈ {rupees(c.usd, usdInr)} · {c.note}</span>
          </div>
        ))}
      ...
      <div className="usage-note">
        ...
        <p>
          ...
          the Anthropic Console — this app cannot read it. Rupees shown at ₹{usdInr} per $, which
          you can change with the <code>USD_INR</code> variable.
        </p>
      </div>
            ...
            {days.map((d) => {
              ...
              return (
                <li key={`${d.day}-${d.model}`}>
                  ...
                  <span className="u-cost">{money(d.usd)}</span>
                </li>
              );
            })}
          </ul>
