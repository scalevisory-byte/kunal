...
### Frontend (React)
- Dashboard: a **four-up clickable stat board** (open / in progress / overdue / done) — each cell is a view — then a **wide column board**, up to 1280px. Columns are **Overdue / Today / Tomorrow / Day after / Later / No date**, switchable to **one column per chat** (the choice is remembered per browser). A task is **one compact row** — checkbox, title, date chip; tapping the title opens description, source and the edit controls. Descriptions that merely restate the title are suppressed, in the UI and in the extraction prompt. Filter open/done/all, manual add, mark done, delete. Polls every 30s.
