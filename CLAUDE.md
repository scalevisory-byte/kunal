...
- The task row shows **Deadline** and **Follow-up** directly, so neither needs the drawer opened. `backend/tests/history.test.mjs` (17 cases) covers the log, waiting, archive and the history view.
...
- **Off by default** — it is the only scheduled thing that costs money every day whether or not anything was published. Toggle, time, "Fetch now" (build without sending) and "Send now" live on their own sidebar page, **Automation → Law updates**, with the last week's digests folded underneath. It started at the bottom of the reminder settings and was simply not found there — everything else on that page is about his own tasks, and this is not about tasks at all.
