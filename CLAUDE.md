...
- Tasks live in one sheet, split by **collapsible tinted section bands** — Overdue (red), Today (amber), Tomorrow (blue), Upcoming, No date — switchable to **one section per chat**. A row is a title, a meta line (chat, AI-or-manual, priority) and a date chip, with everything else behind a **⋮ menu** (Start, Due today, Due tomorrow, Details, Delete) that appears on hover and stays visible on touch. Descriptions that merely restate the title are suppressed, in the UI and in the extraction prompt.
