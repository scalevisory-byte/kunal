...
- **Reminders are their own table.** A task or follow-up can carry several; each has a lifecycle (scheduled → triggered → acknowledged, or snoozed, cancelled, missed). `tasks.remind_at` survives as the *next* reminder for display and for the digest, kept in step by `task-reminders.js` — the `reminders` table is what the engine reads, so there is one source of truth about when to fire.
- **Duplicates are impossible by construction.** A partial unique index on `(task, follow-up, fire_at)` for active rows means scheduling the same moment twice returns the existing reminder. Delivery claims a row with a conditional `UPDATE ... WHERE status IN ('scheduled','snoozed')` — exactly one caller can win, so a second tick, a restart mid-send, or a retry finds nothing to claim. Snooze **moves** the row rather than adding one.
- **One scheduler, not three.** The engine runs on the existing `EXACT_REMINDER_CRON` tick alongside the two digests. Reminders older than `missedAfterHours` (default 24) are marked *missed* rather than fired late. A completed task cancels everything still scheduled for it.
- **Follow-ups** (`follow_ups` + `follow_up_events`) are a separate concept: chasing somebody else, not work you do. Statuses waiting / due / overdue / snoozed / needs_attention / completed / cancelled, with optional `interval_days` + `max_follow_ups`; after the maximum the chain stops at **needs attention** rather than nagging forever. Every change is recorded as history.
- **A reply pauses the chase but never closes it.** An incoming message in a chat with an open follow-up records `responded_at` and cancels its reminders — only the user knows whether the reply actually answered the question.
- **Nothing is ever sent to a contact.** Reminders go to the in-app notification centre and (when enabled) a browser push; the WhatsApp channel is off by default and, even on, only messages Dinesh's own chat.
- The extraction schema gained `kind` (task | follow_up) and `follow_up_reason`, with the prompt told to prefer `task` when unsure — a wrong follow-up keeps asking about something nobody is waiting for.
- Settings (business hours, skip weekends, missed window, follow-up interval and maximum, escalation, channels) live in the `meta` table and have their own section in Settings. `backend/tests/scheduling.test.mjs` (30 cases, `npm test`) covers duplicates, claiming, snooze, cancellation, timezone, quiet hours and the follow-up lifecycle.
